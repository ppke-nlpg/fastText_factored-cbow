...
void FastText::loadModel(std::istream& in)
{
    ...
    {
        std::vector<int64_t> tmpVec1 = dict_->getCounts(entry_type::word);
        std::vector<int64_t> tmpVec2 = dict_->getCounts(entry_type::factor);
...
void FastText::cbow(Model& model, real lr,
                    const std::vector<std::vector<int32_t>>& line)
{
    /*
       std::cout << "DEBUG LINE:\n";
       for(auto word: line){
    ...
    */
    ...
    for (int32_t w = 0; w < line.size(); w++)
    {
        ...
        for(const int32_t & wordOrFactor : line[w])
        {
...
void FastText::trainThread(int32_t threadId)
{
    ...
    std::vector<int32_t> line, labels;
    std::vector<std::vector<int32_t>> factoredLine;
    while (tokenCount_ < args_->epoch * ntokens)
    {
        ...
        {
            localTokenCount += dict_->getLine(ifs, factoredLine, model.rng);
            cbow(model, lr, factoredLine);
        }
