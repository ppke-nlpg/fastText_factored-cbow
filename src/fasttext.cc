...
void FastText::loadModel(std::istream& in) {
  ...
  if (args_->model == model_name::sup) {
        ...
        auto tmpVec1 = dict_->getCounts(entry_type::word);
        auto tmpVec2 = dict_->getCounts(entry_type::factor);
        tmpVec1.insert(tmpVec1.end(), tmpVec2.begin(), tmpVec2.end());
        model_->setTargetCounts(tmpVec1);
    }
}
...
{/*
    std::cout << "DEBUG LINE:\n";
    for(auto word: line){
        std::cout << dict_->getWord(word) << " " << word << "\t";
        for(auto sub : dict_->getSubwords(word))
            std::cout << " " << sub;
    }
    std::cout << std::endl;
*/
    std::vector<std::vector<int32_t>> tmpLine;
    for(auto word: line)
    {
        if(dict_->getType(word) == entry_type::word)
            tmpLine.push_back(std::vector<int32_t>(1,word));
        else if(tmpLine.size() > 0 && dict_->getType(word) == entry_type::factor)
            tmpLine[tmpLine.size()-1].push_back(word);
    }
/*
    std::cout << "TMPLINE:\n";
    for(unsigned int i=0; i<tmpLine.size(); ++i)
    {
        for(unsigned int j=0; j<tmpLine[i].size(); ++j)
        {
            std::cout << tmpLine[i][j] << " ";
        }
        std::cout << std::endl;
    }
*/
    ...
    for (int32_t w = 0; w < tmpLine.size(); w++)
    {
        int32_t boundary = uniform(model.rng);
        bow.clear();
        for (int32_t c = -boundary; c <= boundary; c++)
        {
            if (c != 0 && w + c >= 0 && w + c < tmpLine.size())
            {
                const std::vector<int32_t>& ngrams = dict_->getSubwords(tmpLine[w + c][0]);
                bow.insert(bow.end(), ngrams.cbegin(), ngrams.cend());
            }
        }

        for(int32_t & wordOrFactor : tmpLine[w])
        {
            model.update(bow, wordOrFactor, lr);
        }
